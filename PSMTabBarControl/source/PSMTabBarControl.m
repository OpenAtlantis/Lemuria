...
- (void)addTabViewItem:(NSTabViewItem *)item atIndex:(int)index
{
    // create cell
    PSMTabBarCell *cell = [[PSMTabBarCell alloc] initWithControlView:self];
    [cell setRepresentedObject:item];
...
- (void)orderCellsToMatch
{
    ...
    NSEnumerator *tabItemEnum = [tabItems objectEnumerator];
    while (item = [tabItemEnum nextObject]) {
        NSEnumerator *cellEnum = [_cells objectEnumerator];
        PSMTabBarCell *cell = nil;
        PSMTabBarCell *result = nil;
        while (!result && (cell = [cellEnum nextObject])) {
...
- (void)mouseDown:(NSEvent *)theEvent
{
	...
	if (([nextEvent type] == NSLeftMouseDragged) && (![[NSUserDefaults standardUserDefaults] boolForKey:@"lemuria.dragging.disabled"])) {
        ...
        NSRect dragFrame;
        PSMTabBarCell *dragCell = [self cellForPoint:clickPoint cellFrame:&dragFrame];

        if (dragCell && [[self delegate] respondsToSelector:@selector(tabView:willDragTabViewItem:withEvent:)]) {
            [[self delegate] tabView:tabView willDragTabViewItem:[dragCell representedObject] withEvent:theEvent];            
        }
    }
...
{
    NSMutableArray *temp = [NSMutableArray arrayWithArray:_cells];
    return temp;
}
