...
- (void)setBackgroundPattern:(NSImage *)image {
	if (![self couplingSplitView]) {
		[background autorelease];
        isPattern = YES;
		background = [[NSColor colorWithPatternImage:image] retain];
		[self setNeedsDisplay:YES];
	}
...
// thumbs.
	if (leading||trailing) {
		...
		if (bg) {
            ...
            NSImage *pattern = nil;
            if (isPattern)
                pattern = [bg patternImage];

            if (pattern) {
                NSSize imageSize = [pattern size];
                
                NSPoint origin = rect.origin;
                
                if ([self isHorizontal]) {
                    origin.y -= ((imageSize.height - [self dividerThickness]) / 2);
                }
                else {
                    origin.x -= ((imageSize.width - [self dividerThickness]) / 2);
                }
